    ...
    <div class="content">
      ...
      <div
        ...
        <n-card class="siderItem adminInfo">
          <n-divider class="title"> 博主信息 </n-divider>
          <n-spin :show="state.getAdminInfoLoading">
            ...
            <div
              ...
              <div>
                <n-avatar
                  class="avatar"
                  :src="state.adminInfo.avatarUrl"></n-avatar>
              </div>
        ...
        <n-card
          ...
          <n-divider class="title"> 快捷操作 </n-divider>
          <n-space justify="center">
            <router-link
              to="/blog/article/add"
              v-if="store.state.user.adminMode">
              <n-button type="primary"> 发表文章 </n-button>
            </router-link>
            <router-link
              v-if="store.state.user.adminMode"
              :to="{
            ...
            <router-link
              ...
              <n-button> 我的收藏 </n-button>
            </router-link>
          </n-space>
        </n-card>
        <Notice />
        <n-card class="siderItem">
          <n-divider class="title"> 热门文章 </n-divider>
          <n-spin :show="state.getHotArticleListloading">
            <n-space
              vertical
              v-if="state.hotArticleList.length > 0"
              class="hotArticleList">
              <router-link
                :to="`/blog/article?id=${item.id}`"
                v-for="(item, index) in state.hotArticleList"
                class="articleItem"
                :key="item.id">
                <div class="title">
                  {{ item.title }}
