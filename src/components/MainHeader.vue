    ...
    <n-space align="center">
      ...
      <n-popover
        ...
        <color-picker
          isWidget
          disableAlpha
          :pure-color="store.state.theme.primaryColor"
      ...
      <n-button
        class="toggleDarkModeBtn"
        @click="toggleDarkMode"
        >{{ store.state.theme.darkMode ? "浅色" : "暗色" }}
      </n-button>
      <n-spin
        v-if="store.state.user.userInfoLoading"
        :size="14"
        stroke="#fff"></n-spin>
      <n-space
        v-else-if="!!userInfo"
...
<script lang="ts" setup>
  ...
  const defaultMenu = computed(() => "/" + route.path.split("/")[1]);
